...
| POST | `/v1/zakat-disbursement-distribution/initiate` | Initiate |
| GET | `/v1/zakat-disbursement-distribution` | Retrieve (list) |
| GET | `/v1/zakat-disbursement-distribution/{crId}/retrieve` | Retrieve |
| PUT | `/v1/zakat-disbursement-distribution/{crId}/update` | Update |
| PUT | `/v1/zakat-disbursement-distribution/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/zakat-disbursement-distribution/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-zakat-distribution:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-zakat-distribution ./helm -n isb-governance
```

Exposed through the platform Gateway at path prefix `/isb-zakat-distribution` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
